...
export function middleware(request: NextRequest) {
  ...
  if (isAdminPath) {
    if (!token) {
      console.log('Middleware - Admin path without token, redirecting to login');
      const url = new URL('/login', request.url);
      url.searchParams.set('from', pathname);
      return NextResponse.redirect(url);
    }

    // Check if user is an admin by making a request to /api/users/me
    return fetch(new URL('/api/users/me', request.url), {
      headers: {
        Cookie: `token=${token.value}`,
      },
    }).then(async (response) => {
      if (!response.ok) {
        console.log('Middleware - Failed to verify admin status, redirecting to login');
        const url = new URL('/login', request.url);
        url.searchParams.set('from', pathname);
        return NextResponse.redirect(url);
      }
      ...
      if (!data?.user?.collection || data.user.collection !== 'users') {
        console.log('Middleware - Non-admin user attempting to access admin path, redirecting to home');
        return NextResponse.redirect(new URL('/', request.url));
      }

      return NextResponse.next();
    }).catch(() => {
      console.log('Middleware - Error checking admin status, redirecting to login');
      const url = new URL('/login', request.url);
      url.searchParams.set('from', pathname);
      return NextResponse.redirect(url);
    });
